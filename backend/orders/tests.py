...
class DebtFlowTests(TestCase):
    ...
    def test_debt_payment_flow(self):
        async def run_flow():
            ...
            await order_services.add_payment(order, user=self.user, method="cash", amount=Decimal("20"))
            debt = await order_services.ensure_debt(order)
            payment = await order_services.add_debt_payment(debt, user=self.user, amount=Decimal("20"))
            await debt.arefresh_from_db()
            return order, debt, payment

        order, debt, payment = async_to_sync(run_flow)()
